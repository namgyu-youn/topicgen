...
class TopicAnalyzer:
    ...
    def set_classifier(self):
        try:
            ...
            self.classifier = pipeline(
                "zero-shot-classification", model=self.model, tokenizer=self.tokenizer, device=self.device
            )
        except Exception as e:
            print(f"Error initializing classifier: {e!s}")
            raise
    ...
    async def generate_topics(self, text: str, category: str, subcategory: str) -> list[dict]:
        try:
            ...
            result = self.classifier(text[: self.max_length], all_topics, multi_label=True)
